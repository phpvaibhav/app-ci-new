...
$("#login-form").validate({
  ...
  submitHandler: function (form) {
    ...
    $('#submit').prop('disabled', true);
      $.ajax({
        type        : "POST",
        url         : base_url+'apiv1/api/'+$(form).attr('action'),
        data        : $(form).serialize(),
        cache       : false,
        beforeSend  : function() {
          preLoadshow(true);
        ...
        success: function (res) {
          ...
          if(res.status=='success'){
            ...
            setTimeout(function(){ window.location = base_url+'institute'; },4000);
          }else{
...
//sign up
$("#form-register-front").validate({// Rules for form validation
  rules : {
    name    : {
      required  : true
    },
